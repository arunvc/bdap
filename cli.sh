...
export server_base_url=${BASE_URL}
export server_localhost_base_url=${BASE_URL}
export ae_title=${TITLE}
...
fi# Mailto: arun.vc@infospica.com
...
elif [ "$MY_RUN" == "up" ] || [ "$MY_RUN" == "start" ] || [ "$MY_RUN" == "stop" ] || [ "$MY_RUN" == "restart" ] || [ "$MY_RUN" == "rm" ]; then
docker-compose "$MY_RUN"
...
docker save -o target/${CONTAINER_NAME}.tar ${APP_IMAGE}
...
TIME=$((`date '+%s%N'`/1000))
ssh -p $SSH_PORT -o StrictHostKeyChecking=no $SSH_USER@$SSH_HOST "sudo su;sudo mkdir -p $DEPLOY; cd $DEPLOY; if [ -f ${DOCKER_COMPOSE_FILE} ]; then sudo mv ${DOCKER_COMPOSE_FILE} /tmp/${DOCKER_COMPOSE_FILE}.$TIME; fi"
ssh -p $SSH_PORT -o StrictHostKeyChecking=no $SSH_USER@$SSH_HOST "sudo su; if [ ! -d ${CERTS} ]; then sudo mkdir -p ${CERTS};openssl genrsa -out ${CERTS}/keypair.pem 2048;openssl rsa -in ${CERTS}/keypair.pem -pubout -out ${CERTS}/public.pem;openssl pkcs8 -topk8 -inform PEM -outform PEM -nocrypt -in ${CERTS}/keypair.pem -out ${CERTS}/private.pem; fi"
scp -P $SSH_PORT -o StrictHostKeyChecking=no ./${DOCKER_COMPOSE_FILE} "$SSH_USER@$SSH_HOST:$DEPLOY"
scp -P $SSH_PORT -o StrictHostKeyChecking=no ./cli.sh "$SSH_USER@$SSH_HOST:$DEPLOY"
scp -P $SSH_PORT -o StrictHostKeyChecking=no ./cli-env.sh "$SSH_USER@$SSH_HOST:$DEPLOY"
ssh -p $SSH_PORT -o StrictHostKeyChecking=no $SSH_USER@$SSH_HOST "sudo su;sudo mkdir -p $DEPLOY; cd $DEPLOY;./cli.sh -e \"${BUILD_ENV}\" -c refresh -a \"${DB_USER}\" -s \"${DB_PASSWORD}\" -d \"${DB}\" -n \"${DB_HOST_NAME}\" -o \"${DB_PORT}\" -x \"${EMAIL_USER}\" -y \"${EMAIL_PASSWORD}\" -w \"${EMAIL_HOST}\" -z \"${EMAIL_PORT}\" -b \"${BASE_URL}\" -t \"${TITLE}\" -f \"${WEB_ADMIN_USER}\" -g \"${WEB_ADMIN_PASSWORD}\""
